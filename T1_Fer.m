...
% Homocedásticos 
r_fe = y_dot - x_dot*b_fe;                   
s2_fe = (1./(n-3))*(r_fe'*r_fe);                 
mvc_fe = s2_fe*inv(x_dot'*x_dot);               
ee_fe = sqrt(diag(mvc_fe));                   

tt_fe = (b_fe(2)-1)/ee_fe(2);
pv_fe = 2 * (1 - tcdf(abs(tt_fe), 997));

% Robustos
r_fe = y_dot - x_dot*b_fe;                 
d_fe = diag(r_fe.^2);                         
mvc_wfe = inv(x_dot'*x_dot)*(x_dot'*d*x_dot)*inv(x_dot'*x_dot);     
ee_rfe = sqrt(diag(mvc_wfe));

tt_rfe = (b_fe(2)-1)/ee_rfe(2);
pv_rfe = 2 * (1 - tcdf(abs(tt_rfe), 997)); 

% Agrupados

% código errores agrupados

tt_cfe = (b_fe(2)-1)/ee_cfe(2);
pv_cfe = 2 * (1 - tcdf(abs(tt_cfe), 997));
